"""
实验组（含语义约束）批量评测脚本
结构与 run_baseline_eval.py 完全一致，唯一区别是使用 SemanticText2SQL 替代 BaselineText2SQL
- 读取 data/samples/test_questions.json 中的 25 条测试问题
- 对每条问题调用 SemanticText2SQL 生成 SQL 并执行
- 将结果与参考 SQL 的执行结果做数值对比
- 输出 CSV 到 outputs/reports/experiment_results.csv
- 同时输出易读版 CSV + Markdown 报告
- 最后自动生成基线vs实验组对比报告

用法:
  python3 -m src.evaluation.run_experiment_eval [--limit N] [--output path]
"""
import os
import sys
import csv
...
DEFAULT_OUTPUT = os.path.join(BASE_DIR, "outputs", "reports", "experiment_results.csv")
...
def save_results(results, output_path):
    ...
    fieldnames = [
        ...
        "generated_sql", "matched_metrics", "exec_success", "exec_error", "exec_row_count",
...
def main():
    parser = argparse.ArgumentParser(description="批量跑实验组（含语义层）Text-to-SQL评测")
    parser.add_argument("--limit", type=int, default=0, help="只跑前N题(0=全部)")
    parser.add_argument("--start", type=str, default=None, help="从某个question_id开始")
    parser.add_argument("--output", type=str, default=DEFAULT_OUTPUT, help="输出CSV路径")
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--sleep", type=float, default=0.5, help="每题间休息秒数")
    ...
    print("=" * 70)
    print("  🧪 实验组 Text-to-SQL 批量评测（含指标语义约束）")
    ...
    for i, q in enumerate(questions, 1):
        ...
        try:
            ...
            matched_metrics = ",".join(result.get("matched_metrics", []))
        except Exception as e:
        ...
        mm_icon = f"🎯{matched_metrics}" if matched_metrics else ""
        print(f"   SQL执行: {exec_icon}  结果匹配: {match_icon}  {mm_icon}  {reason}")
        if not exec_success:
    ...
    write_readable_csv(results, os.path.join(output_dir, "experiment_results_readable.csv"))
    write_markdown_report(results, os.path.join(output_dir, "experiment_report.md"))

    # 自动生成对比报告
    try:
        from src.evaluation.compare_report import generate_comparison
        generate_comparison()
        print("✅ 对比报告已生成！")
    except Exception as e:
        print(f"⚠️ 对比报告生成跳过（可能基线结果不存在）: {e}")
